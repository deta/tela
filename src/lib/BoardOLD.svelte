...
<script lang="ts">
  ...
  function onMouseMove(e: MouseEvent) {
    ...
    let underCursor: Writable<IPositionable<any>> | null = null;
    for (let i = 0; i < $visiblePositionables.length; i++) {
      const toCheck = $visiblePositionables[i];
      const _toCheck = get(toCheck);
      if (
        (absX >= _toCheck.x &&
        absX <= _toCheck.x + _toCheck.width &&
        absY >= _toCheck.y &&
        absY <= _toCheck.y + _toCheck.height)
        &&
        ((underCursor === null) ||
        (underCursor !== null && ($stackingOrder.indexOf(_toCheck[POSITIONABLE_KEY]) >= $stackingOrder.indexOf(get(underCursor)[POSITIONABLE_KEY]))))
      ) {
        underCursor = toCheck;
      }
    }
    if (underCursor === null) return;
    console.log("under cursor:", underCursor);
  }
